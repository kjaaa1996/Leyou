...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      addChild: function () {   //更新了新的判断条件，必须要先选中父节点才能新增子节点，新增时先要获取数据库中最后一条数据的id
        //this.$http.get("/item/category/last").then(res=>{
          let child = {
            id: 0,    //res.data.id+1 这样的效果不好，如果数据不连续仍然或出错
            name: '新的节点',
