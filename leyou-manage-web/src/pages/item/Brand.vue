<template>
  <v-card>
    <v-card-title>
      ...
      <v-btn color="error" @click="deleteAllBrand">批量删除</v-btn>
      <!--搜索框，与search属性关联-->
      <v-spacer/>
      <v-flex xs3>
  ...
  export default {
    ...
    methods: {
      ...
      deleteBrand(oldBrand){
        this.$message.confirm("这项操作将永久性的删除这条数据，是否继续?").then(()=>{
            this.$http.delete("/item/brand/delete/"+oldBrand.id).then(()=>{
      ...
      deleteAllBrand() {
        //将selected中的元素的id拼接成一个数组
        const ids = this.selected.map(s => s.id);
        if (ids.length > 0) {
          this.$message.confirm("这项操作将永久性删除这几项数据，是否继续?").then(() => {
            //发送删除请求，并将ids数组中的元素用"-"分隔开
            this.$http.delete("/item/brand/delete/" + ids.join("-")).then(() => {
              this.$message.info("删除成功！");
              this.selected=[];
              this.getDataFromServer();
            }).catch(() => {
