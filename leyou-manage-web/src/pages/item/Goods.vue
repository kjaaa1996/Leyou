...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      deleteGoods(id){
        this.$message.confirm("这项操作将永久性删除这些数据，是否继续?").then(()=>{
      ...
      deleteAllGoods(){
        const deletedGoodsIds=this.selected.map(s=>s.id);
        if (deletedGoodsIds.length>0){
          this.$message.confirm("这项操作将永久性删除这些数据，是否继续?").then(()=>{
            this.$http.delete("/item/goods/spu/"+deletedGoodsIds.join("-")).then(()=>{
              this.$message.info("删除成功!");
              this.getDataFromServer();
            }).catch(()=>{
      ...
      soldOutPut(id){
        //将商品的上下架置为反
        this.$http.put("/item/goods/spu/saleable/"+id).then(()=>{
